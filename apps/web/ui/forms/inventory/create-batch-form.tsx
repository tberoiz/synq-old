...
import { createInventoryBatch } from "@synq/supabase/queries/inventory";
...
export const CreateBatchForm = ({ onSuccess }: { onSuccess?: () => void }) => {
  ...
  const onSubmit = async (data: InventoryFormValues) => {
    try {
      await createInventoryBatch(data.name);
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input
                  placeholder="e.g., 'March 2024 Purchase' or 'Rare Cards'"
                  {...field}
                  autoComplete="off"
                  autoCapitalize="none"
                />
        ...
        <DialogFooter>
          ...
          <Button type="submit">Create a new Batch</Button>
        </DialogFooter>
      </form>
    </Form>
  );
